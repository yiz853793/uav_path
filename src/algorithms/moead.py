...
@dataclass
class Individual:
    x: np.ndarray  # decision vars: [K,2]
    er: EvalResult
    ...
    if n_astar > 0:
        ...
        for f in factors:
            ...
            for k in range(max_paths):
                ...
                collision_fn = lambda p, q: segment_collision(env, p, q, step=0.5)
                base = shortcut_smooth(raw.copy(), n_try=250, rng=rng, collision_fn=collision_fn)
                base = densify_polyline_to_K(base, K)
                base[0] = start
                base[-1] = goal
                # 大图常见：平滑后切角导致不可行；若不可行则回退为仅 densify 的栅格路径，保证至少一条可行骨架
                er = evaluate_path(env, base)
                if not er.feasible:
                    base = densify_polyline_to_K(raw, K)
                    base[0] = start
                    base[-1] = goal
                    er = evaluate_path(env, base)
...
# de-dup very similar backbones
    ...
    if astar_paths:
        for t in range(n_astar):
            ...
            x[0] = start
            x[-1] = goal
            x = repair(env, x, tries=18)
            er = evaluate_path(env, x)
...
def mutate(rng: np.random.Generator, x: np.ndarray, sigma: float = 2.5, p_mut: float = 0.25) -> np.ndarray:
    ...
    for i in range(1, len(y) - 1):
        if rng.random() < p_mut:
            y[i] += rng.normal(0.0, sigma, size=(2,)).astype(np.float32)
    # 首尾不变
    y[0] = x[0]
    y[-1] = x[-1]
    return y
...
def repair(env: GridEnv, x: np.ndarray, tries: int = 15) -> np.ndarray:
    """
    简单修复：
    - 裁剪边界
    - 如果点落在障碍上，随机抖动找一个空位置
    ...
    for i in range(1, len(y) - 1):
        if env.is_occupied(int(round(y[i, 0])), int(round(y[i, 1]))):
            for _ in range(int(tries)):
                cand = y[i] + rng.normal(0.0, 4.0, size=(2,)).astype(np.float32)
                cand = _clip_bounds(env, cand[None, :])[0]
                if not env.is_occupied(int(round(cand[0])), int(round(cand[1]))):
                    y[i] = cand
                    break
    return y.astype(np.float32)
...
def moead(
    ...
    for gen in range(int(n_gen)):
        ...
        for i in range(pop):
            ...
            child = repair(env, child, tries=12)

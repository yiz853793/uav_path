...
def default_start_goal_for_env(env: GridEnv) -> Tuple[np.ndarray, np.ndarray]:
    # 你小地图默认：start=[5,5], goal=[140,90]
    # 观察可知：goal 大约等于 [W-60, H-70]
    W, H = env.W, env.H
    sx, sy = 5.0, 5.0
    gx, gy = float(max(0, W - 60)), float(max(0, H - 70))
    # 防止太小地图时 goal 落在 start 附近
    gx = max(gx, sx + 10.0)
    gy = max(gy, sy + 10.0)
...
def save_paths_figure(height, occupancy, start, goal, path_rrt, arch, reps, out_png, title=""):
    fig, ax = plt.subplots(figsize=(9, 6))

    # 底图：越高越深
    im = ax.imshow(height, origin="lower", cmap="viridis_r")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="height")

    # === 叠加禁飞区（不可行区域） ===
    # occupancy: True/1 = no-fly / obstacle, False/0 = free
    ...
    if reps is not None and arch is not None and len(arch.items) > 0:
        idx_map = {
        ...
        for label, idx in idx_map.items():
            p = arch.items[idx].x
            ax.plot(p[:, 0], p[:, 1], linewidth=2.0, label=label)

    # 图例里加入禁飞区说明
    handles, labels = ax.get_legend_handles_labels()
    if occupancy is not None:
        handles.append(Patch(facecolor="red", edgecolor="red", alpha=0.35, label="No-fly zone"))
...
def summarize_results(all_rows: list, log_path: str, args):
    ...
    # RRT* objective stats (only for cases with a found path)
...
def main():
    ...
    if args.glob.strip():
        ...
        terrain_files = [
            os.path.join(args.terrain_dir, f"mountain_seed{seed:04d}.npz")
    ...
    for tp in terrain_files:
        ...
        if args.start is None or args.goal is None:
            start, goal = default_start_goal_for_env(env)
        else:
        ...
        # ===== 文件名：追加 size_tag + moead_core(gen,pop,K,T) + astar_tag =====
        base_noext = base.replace(".npz", "")
        file_tag = f"{size_tag}{moead_core_tag}{astar_tag}"
        fig_path = os.path.join(out_dir, f"paths_{base_noext}_pseed{args.planner_seed:04d}_{file_tag}.png")
        title = f"Paths | {base_noext} size={size_tag} gen={args.moead_gen} pop={args.moead_pop} K={args.K} planner_seed={args.planner_seed}"
        save_paths_figure(height, occupancy, start, goal, path_rrt, arch, reps, fig_path, title=title)

        if pareto_objs is not None:
            pareto_json = os.path.join(out_dir, f"pareto_points_{base_noext}_pseed{args.planner_seed:04d}_{file_tag}.json")

            rep_payload = {
                "min_f1": {
                    "index": int(reps["min_f1"]),
                    "point": [float(x) for x in pareto_objs[int(reps["min_f1"])].tolist()],
                },
                "min_f2": {
                    "index": int(reps["min_f2"]),
                    "point": [float(x) for x in pareto_objs[int(reps["min_f2"])].tolist()],
                },
                "min_f3": {
                    "index": int(reps["min_f3"]),
                    "point": [float(x) for x in pareto_objs[int(reps["min_f3"])].tolist()],
                },
                "compromise": {
                    "index": int(reps["compromise"]),
                    "point": [float(x) for x in pareto_objs[int(reps["compromise"])].tolist()],
                    "weights": [float(x) for x in reps["weights"].tolist()],
                    "score": float(reps["score"][int(reps["compromise"])]),
                },
            }
        ...
        data = {
            ...
            "rrt": {
                ...
                "path_len": float(np.sum(np.linalg.norm(np.diff(path_rrt, axis=0), axis=1))) if path_rrt is not None else None,
